...
-- Demo: vehiculos_gorda con 3 chunks -> con drop_after 5 dias se purga el
...
SELECT count(*) AS total_filas FROM vehiculos_gorda;  -- 2.706.126 (518.405 + 518.405 + 1.669.321... ajustado al estado)

-- ---------------------------------------------------------------
-- 1) MANUAL - ver qué se borraría y borrarlo a mano (una vez, sin job)
SELECT show_chunks('vehiculos_gorda', older_than => now() - interval '5 days');
--    -> devuelve solo 'particiones_gordas.vehiculos_gorda_6003_chunk'
...
SELECT count(*) AS total_filas FROM vehiculos_gorda;  -- menos las filas del chunk purgado (518.405)

-- 6) Si cambias de opinión: eliminar la política de retención
SELECT remove_retention_policy('vehiculos_gorda');